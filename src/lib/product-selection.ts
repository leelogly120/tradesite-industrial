...
function reference(
  ...
  orientation: Orientation,
  authoredRelatedSlugs: readonly string[],
  options: Partial<Pick<ProductReference, 'status' | 'statusNote' | 'scopeStatement' | 'buyerQuestion' | 'confirmationGate' | 'editorialImages' | 'defaultCompare'>> = {},
): ProductReference {
  return {
    slug,
    model,
    ...
    defaultCompare: options.defaultCompare ?? false,
  };
}
