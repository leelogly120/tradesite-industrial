...
import { AUGUST_10_LIFT_PLATFORM_ARTICLES, DAILY_LIFT_PLATFORM_ARTICLES, LIFT_PLATFORM_ARTICLES } from '../scripts/lift-platform-article-registry.mjs';
...
describe('public asset manifest', () => {
  ...
  it('preserves the original 15 editorial assets and publishes the 54 approved long-form assets', async () => {
    const manifest = JSON.parse(await readProjectFile('public/images/asset-manifest.json'));
    ...
    expect(new Set(records.map((record) => record.slug)).size).toBe(69);
    expect(new Set(records.map((record) => record.url)).size).toBe(69);
